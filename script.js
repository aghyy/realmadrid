...
const SportScore_API = "b942e7626dmsh613d16ecec8d823p1aa1e6jsn0b03af724864";
...
document.addEventListener("scroll", function() {
	if (!document.querySelector(".fullscreen")) {
		...
		if (window.pageYOffset >= headerHeight) {
			scrollTopBtn.style.display = "flex";
			obligatoryElem.style.display = "block";
		} else {
			scrollTopBtn.style.display = "none";
...
if (document.title === "Real Madrid") {
	...
	const options = {
		method: 'GET',
		headers: {
			'X-RapidAPI-Key': SportScore_API,
			'X-RapidAPI-Host': 'sportscore1.p.rapidapi.com'
		}
	};
	
	fetch('https://sportscore1.p.rapidapi.com/teams/139/events', options)
		.then(response => response.json())
		.then(response => {
			response.data.reverse().forEach(match => {
